...
if (1) {			# do named entity recognition
  ...
  $dontskip->{NamedEntityRecognition} = 1;
} else {			# don't do named entity recognition
  $skipped->{NamedEntityRecognition} = 1;
  delete $dontskip->{NamedEntityRecognition};
}
...
foreach my $cause (sort {$causes->{$b} <=> $causes->{$a}} keys %$causes) {
  ...
  my $res123 = ProcessTextWithManufacturersList
    (
     Text => $latinified,
    );
  print Dumper($res123);
